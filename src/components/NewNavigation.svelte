...
<header id="header">
	...
	<nav id="header__navigation">
		...
		{:else}
			<div id="login" on:click={logout}>
				<span
					class="login"
					on:mouseenter={() => {
						tempHoverable = hoverable;
						hoverable = "Odhlásit se";
					}}
					on:mouseleave={() => {
						hoverable = tempHoverable;
					}}>{hoverable}</span
				>
			</div>
		{/if}
...
{#if showRegister || showLogin || showCode}
	<div
		id="darkener"
		on:click={() => {
			showRegister = false;
...
{#if showPrihlaseni}
	<div id="login-phone" transition:fly={{ x: 500, duration: 500 }}>
		...
		<u id="login-forgot-phone">Zapomenuté heslo</u>
		<svg
			id="login-logo-phone"
			xmlns="http://www.w3.org/2000/svg"
			width="82"
...
{#if showNav || showPrihlaseni || showRegistrace || showKod}
	<div
		id="nav-bg"
		on:scroll={() => {
			showNav = false;
